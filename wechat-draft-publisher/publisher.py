...
class WeChatPublisher:
    ...
    def load_config(self):
        ...
        if not self.appsecret or self.appsecret in [
            "your_appsecret_here",
            "your_appsecret",
        ]:
            raise ValueError(
                f"请在配置文件中填写有效的appsecret\n配置文件: {self.CONFIG_FILE}"
            )
    ...
    def _fix_wechat_editor_issues(self, content: str) -> str:
        ...
        def convert_bg_div_to_table(match):
            ...
            if (
                "background:#ffffff" in style_no_space
                or "background-color:#ffffff" in style_no_space
            ) and "border" not in style:
                conversion_count["excluded"] += 1
                return match.group(0)
        ...
        # 打印转换统计
        print(
            f"  → 背景色区块转换: 成功转换 {conversion_count['converted']} 个, 排除 {conversion_count['excluded']} 个"
        )
        ...
        content = re.sub(
            r"border-radius:\s*([^;!]+);", r"border-radius:\1!important;", content
        )
        ...
        content = re.sub(
            r"\bbackground:\s*([#a-fA-F0-9]+);",
            r"background-color:\1!important;",
            content,
        )

        # 4.3 确保所有 background-color 都有 !important
        content = re.sub(
            r"background-color:\s*([^;!]+);", r"background-color:\1!important;", content
        )
        ...
        content = re.sub(
            r"margin-bottom:\s*([3-9]\d+|[1-9]\d{2,})px;",
            "margin-bottom:18px;",
            content,
        )  # 只压缩>=30px的
        content = re.sub(
            r"margin:\s*\d+px\s+0\s+\d+px\s+0;", "margin:0 0 18px 0;", content
        )
        ...
        content = re.sub(
            r"text-indent:\s*[^;!]+;", "text-indent: 0 !important;", content
        )
        ...
        # 段落的 margin 设置为舒适值
        content = re.sub(
            r"margin:\s*0\s+0\s+\d+px\s+0;", "margin: 0 0 18px 0 !important;", content
        )
        content = re.sub(
            r"margin-bottom:\s*\d+px;", "margin-bottom: 18px !important;", content
        )
        # section/卡片 之间的间距（调整为18px，更自然）
        content = re.sub(
            r"margin:\s*\d+px\s+0;", "margin: 0 0 18px 0 !important;", content
        )

        # 3. 添加 !important 到关键对齐属性
        # vertical-align（防止圆形序号错位）
        content = re.sub(
            r"vertical-align:\s*([^;!]+);", r"vertical-align: \1 !important;", content
        )

        # text-align（防止对齐错位）
        content = re.sub(
            r"text-align:\s*([^;!]+);", r"text-align: \1 !important;", content
        )

        # 4. display 属性加 !important（防止布局打乱）
        content = re.sub(
            r"display:\s*inline-block;", "display: inline-block !important;", content
        )

        # 5. line-height 加 !important（防止行高被重置）
        content = re.sub(
            r"line-height:\s*([^;!]+);", r"line-height: \1 !important;", content
        )

        # 6. font-size 加 !important（防止字体变大）
        content = re.sub(
            r"font-size:\s*([^;!]+);", r"font-size: \1 !important;", content
        )

        # 7. padding 加 !important（防止内边距变化）
        content = re.sub(r"padding:\s*([^;!]+);", r"padding: \1 !important;", content)
        ...
        content = re.sub(r"border-top:\s*1px\s+dashed\s+#ccc;", "", content)
...
使用示例:
  ...
  %(prog)s --interactive  # 交互式模式
    ...
    parser.add_argument(
        "--cover", default="cover.png", help="封面图片路径（默认: cover.png）"
    )
